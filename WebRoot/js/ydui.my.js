...
//          console.error('请在ydui.js前引入ydui.citys.js。下载地址：http://cityselect.ydui.org');
    ...
    CitySelect.prototype.open = function () {
        ...
        _this.loadCountry();
        _this.loadProvance();
        console.log(defaultSet.r_c);
        if (defaultSet.provance) {
            _this.setNavTxt(0, defaultSet.provance, defaultSet.r_c.split('-')[0]);
        } else {
        ...
        if (defaultSet.city) {
            _this.loadProvance();
            _this.setNavTxt(1, defaultSet.city, defaultSet.r_c.split('-')[1]);
        }

        if (defaultSet.county) {
            _this.loadCity();
            _this.ForwardViewTwo(true);
            _this.setNavTxt(2, defaultSet.county, defaultSet.r_c.split('-')[2]);
        }

        if (defaultSet.town) {
            _this.loadArea();
            _this.ForwardViewTwo(true);
            _this.setNavTxt(3, defaultSet.town, defaultSet.r_c.split('-')[3]);
        }
    ...
    CitySelect.prototype.createDOM = function () {
        ...
        _this.$cityElement = $('' +
            ...
            '        <p class="cityselect-title">所在城市</p>' +
    ...
    CitySelect.prototype.setNavTxt = function (index, txt, r_c) {
    ...
    CitySelect.prototype.bindItemEvent = function () {
        ...
        $cityElement.on('click.ydui.cityselect', '.cityselect-item-box a', function () {
            ...
            var r_c = $this.attr("cid");
            _this.setNavTxt(tag, $this.text(), r_c);
            ...
            if (tag == 0) {
                ...
                defaultSet.county = $nav.eq(2).html();
                defaultSet.town = $nav.eq(3).html();
                defaultSet.r_c = $nav.eq(0).attr("cid") + '-' + $nav.eq(1).attr("cid") + '-' + $nav.eq(2).attr("cid") + '-' + $nav.eq(3).attr("cid");
                console.log(defaultSet.r_c)
                _this.returnValue();
            }
        });
    ...
    CitySelect.prototype.returnValue = function () {
        ...
        _this.$element.trigger($.Event('done.ydui.cityselect', {
            ...
            county: defaultSet.county,
            town: defaultSet.town,
            r_c: defaultSet.r_c
        }));
    ...
    CitySelect.prototype.loadCountry = function () {
        ...
        $.each(_this.citys, function (k, v) {
            arr.push($('<a cid="' + v.r_c + '" class="' + (v.r_n == _this.defaultSet.provance ? 'crt' : '') + '" href="javascript:;"><span>' + v.r_n + '</span></a>').data({
                citys: v.r_c_l,
                tag: 0
            }));
        });
    ...
    CitySelect.prototype.loadProvance = function () {
        ...
        if (countryData == undefined) {
            ...
            $.each(countryData, function (k, v) {
                arr.push($('<a cid="' + v.r_c + '" class="' + (v.r_n == _this.defaultSet.city ? 'crt' : '') + '" href="javascript:;"><span>' + v.r_n + '</span></a>').data({
                    citys: v.r_co_l,
                    tag: 1
                }));
            });
    ...
    CitySelect.prototype.loadCity = function () {
        ...
        if (cityData == undefined) {
            ...
            $.each(cityData, function (k, v) {
                arr.push($('<a cid="' + v.r_c + '" class="' + (v.r_n == _this.defaultSet.county ? 'crt' : '') + '" href="javascript:;"><span>' + v.r_n + '</span></a>').data({
                    citys: v.r_t_l,
                    tag: 2
                }));
            });
    ...
    CitySelect.prototype.loadArea = function () {
        ...
        if (areaData == undefined) {
            ...
            $.each(areaData, function (k, v) {
                arr.push($('<a cid="' + v.r_c + '" class="' + (v.r_n == _this.defaultSet.town ? 'crt' : '') + '" href="javascript:;"><span>' + v.r_n + '</span></a>').data({tag: 3}));
            });
        }
...
;(function () {
    ...
    FastClick.notNeeded = function (layer) {
        ...
        if (firefoxVersion >= 27) {
            // Firefox 27+ does not have tap delay if the content is not zoomable - https://bugzilla.mozilla.org/show_bug.cgi?r_c=922896
