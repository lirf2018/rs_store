...
public class SecurityCode extends HttpServlet {
    private Logger LOG = Logger.getLogger(SecurityCode.class);
    ...
    public void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        for (int i = 0; i < 4; i++) {
            ...
            sRand = (new StringBuilder(String.valueOf(sRand))).append(rand).toString();//纯数字
            g.setColor(new Color(20 + random.nextInt(130), 20 + random.nextInt(130), 20 + random.nextInt(130)));
            g.drawString(rand, 13 * i + 6, 16);
        }
        request.getSession().setAttribute("imgCode", sRand);
        g.dispose();
        ImageIO.write(image, "PNG", response.getOutputStream());
        LOG.info("------------>生成图形码：" + sRand);
    }
